...
public class BackFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        log.info("===Reader Back Platform access do filter begin===");
        ...
        log.info("===url------->>>>>>" + url +"===");
        String key = CommonConstant.SIGN_NAME_KEY;
            ...
            if (timestamp != null && signInfo != null) {
                ...
                if (-30L <= s && s <= 30L) {
                    ...
                    if (!signInfo.equals(MD5Util.code(code))) {
                       ...
                       if(!url.contains("/api/reader/back/adminLogin")
                               && !url.contains("/api/reader/back/advertiserLogin")
                               && !url.contains("/api/reader/back/mediaOwnerLogin")){
                            log.info("=== request begin filter === token:" + token);
                            if (token != null && !"".equals(token)) {
                                 if (redisClient.hasKey(token)){
                                    TokenUserVO tokenUserVo = (TokenUserVO) redisClient.get(token,TokenUserVO.class);
                                    log.info("===tokenUserVo:" + tokenUserVo);
                                    redisClient.set(token,tokenUserVo,1,TimeUnit.DAYS);
                                    log.info("=== request filter end ===");
                                }else {
                                    log.info("=== request token is miss end ===");
                                    rew2Result.setCode(101);
                                    rew2Result.setMessage("未登陆");
                                    errorResponse(response, jsonUtil.toString(rew2Result));
                                    return;
                                }
                            } else {
                           ...
                           log.info("===app request begin filter ===");
                        }
                    }
                } else {
